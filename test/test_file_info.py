...
files_authors = [
    (
        ("dir1/file1.txt", "dir1/file2.pdf", "dir2/file3.py", "dir1/dir12/file4.doc"),
        {
...
def load_files(conn: sqlite3.Connection, files):
    ...
    for file in files:
        ...
        curs.execute(
            "insert into Files (DirID, FileName) values (?, ?);", (str(d_id), f_name)
        )
...
def test_insert_author(init_load_obj, files, authors_dict):
    ...
    for key, authors in authors_dict.items():
        ...
        for author in authors:
            ...
            aa = curs.execute(
                "select Author from Authors where AuthorID = ?", (str(a_id),)
            ).fetchone()
            assert aa[0] == author
