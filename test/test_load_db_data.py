...
def test_insert_file(init_load_obj, dirs, files):
    ...
    for file in files:
        ...
        assert (
            cnt[0] == 1
        ), f"file {file[0]} must be inserted only once in one directory"
        res = curs.execute(
            "select dirid from files where filename = ?;", file[:1]
        ).fetchall()
        assert (
            dir_ids[file[1]][0],
        ) in res, f"file {file[0]} must be inserted, but not"
...
def test_insert_extension(init_load_obj, files):
    ...
    for file in files:
        ...
        assert (
            id > 0
        ), "method must return extension ID > 0  either inserted now or before now"
        ext = conn_d.execute(
        ...
        assert ext[0][0] == p_file.suffix.strip(
            "."
        ), f"extension of file {p_file} is not {ext[0][0]}"
...
def test_find_old_parent_id(child_parent_fixture):
    ...
    for dir_ in dir_parent:
        ...
        if dir_[2]:
            assert (
                not res == dir_ids[dir_[1]][1]
            ), f"{dir_[0]} must not be parent for {res}"
        else:
...
def test_change_parent(child_parent_fixture):
    ...
    for dir_ in dir_parent:
        ...
        if inserted:
            ...
            for cc in curs:
                assert str(cc[1]).startswith(
                    dir_[0]
                ), f"child path must start with {dir_[0]}"
